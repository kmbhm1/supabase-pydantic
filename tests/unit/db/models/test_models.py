"""Tests for database models in supabase_pydantic.db.models."""
...
def test_ConstraintInfo_methods():
    ...
    # Check string representation directly instead of parsing as JSON
    assert str(constraint_info) == 'ConstraintInfo(test, PRIMARY KEY)'

    # Check as_dict method for fields access
    constraint_dict = constraint_info.as_dict()
    assert constraint_dict['constraint_name'] == 'test'
    assert constraint_dict['raw_constraint_type'] == 'p'
    assert constraint_dict['constraint_definition'] == 'PRIMARY KEY (foo)'
    assert constraint_dict['columns'] == ['foo', 'bar']
