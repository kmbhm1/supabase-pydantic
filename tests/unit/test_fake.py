...
from src.supabase_pydantic.db.seed.fake import (
    generate_fake_data,
...
def fake_faker():
    ...
    with patch(
        'src.supabase_pydantic.db.seed.fake.Faker'
...
def test_nullable_field(fake_faker, monkeypatch):
    ...
    monkeypatch.setattr(
        'src.supabase_pydantic.db.seed.fake.random', lambda: 0.01
    )  # Below the threshold to simulate null
